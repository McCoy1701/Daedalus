...
static double _sisyphus_loop_start_time = 0.0;
...
#define LOOP_TEST_START() \
    do { \
        ...
        _sisyphus_loop_start_time = get_time_precise(); /* Start timer for the loop */ \
        printf(TEST_COLOR_CYAN "  🌀 Entering Repetitive Trial..." TEST_COLOR_RESET "\n"); \
    } while(0)

#define LOOP_TEST_END() \
    do { \
        double _sisyphus_loop_end_time = get_time_precise(); /* End timer for the loop */ \
        double _sisyphus_loop_elapsed_time = _sisyphus_loop_end_time - _sisyphus_loop_start_time; \
